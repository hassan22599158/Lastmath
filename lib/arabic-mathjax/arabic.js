/*!
 * Arabic MathJax Extension - v3.1.0 (Text Mirroring Fix)
 * Fixes: \text{} inside \alwaysar now correctly un-flips.
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* كلاس التصحيح: يقوم بقلب العنصر مرة أخرى ليعود طبيعياً */
        '.MathJax .munflip': {
          'display': 'inline-block !important',
          '-moz-transform': 'scaleX(-1)',
          '-webkit-transform': 'scaleX(-1)',
      ...
      MML[name].Augment({
        toHTML: function () {
          ...
          if (this.arabicFlipH) {
            ...
            // إذا كان العنصر محمياً (نص أو استثناء) نستخدم munflip
            if (this.unflip) {
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  TEX.Definitions.Add({
    macros: {
      ...
      'noar': 'HandleNoArabic',
      'textar': 'HandleTextAr',
      'text': 'HandleTextAr' // <--- التعديل الجوهري: استبدال الأمر text العادي بالأمر الذكي
    }
  });
  ...
  TEX.Parse.Augment({
    ...
    HandleTextAr: function(name) {
        ...
        // الحماية: إذا كنا في وضع عربي، طبق unflip
        if ('ar' === this.stack.env.lang) {
             mtext.unflip = true;
