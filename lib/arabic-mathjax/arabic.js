 ...
 *
 * Modified to ADD \txt command ONLY.
 * Original logic for \alwaysar and flipHorizontal is UNTOUCHED.
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* --- 1. الإضافة الجديدة: CSS للأمر txt --- */
        /* هذا الكلاس لا يفعل شيئاً في الوضع العادي */
        '.MathJax .ar-txt-fix': {
          'display': 'inline-block !important'
        },
        /* ولكن: إذا كان داخل حاوية مقلوبة (mfliph)، نقوم بعكسه مرة أخرى ليصبح مقروءاً */
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  TEX.Definitions.Add({
    macros: {
      ...
      'txt': 'HandleTxt'   // --- 2. الإضافة الجديدة: تسجيل الأمر txt ---
    }
  });
  ...
  TEX.Parse.Augment({
    ...
    // --- 3. الإضافة الجديدة: دالة معالجة الأمر txt ---
    HandleTxt: function(name) {
        var arg = this.GetArgument(name);
        // نقوم بعمل TeX جديد: class + text فقط
        // لا نستخدم fliph ولا نتدخل في ترتيب المكتبة
        // نحن فقط نضع "علامة" (class) على النص
