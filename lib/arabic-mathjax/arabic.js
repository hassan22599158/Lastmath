/*!
 * Arabic MathJax Extension - Golden Master
 * Concept: Global Environment Inheritance.
 * Result: EVERYTHING inside \alwaysar is translated (Numbers, Symbols, Matrices, etc).
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* كلاس لحماية النصوص من القلب العكسي */
        '.MathJax .munflip': {
          'display': 'inline-block !important',
          '-moz-transform': 'scaleX(-1)',
          '-webkit-transform': 'scaleX(-1)',
...
MathJax.Hub.Register.StartupHook('HTML-CSS Jax Ready', function () {
  MathJax.Hub.Register.StartupHook('Arabic TeX Ready', function () {
    ...
    // القائمة الشاملة للعناصر التي يجب أن تدعم القلب
    // إضافة 'mtd' هنا هو ما يصلح محتوى المصفوفات
    [
      'mfrac', 'mi', 'mn', 'mo', 'mrow', 'ms', 'msqrt', 'msubsup', 'mroot', 'mtext', 
      'mtd', 'merror', 'mpadded', 'mphantom', 'menclose'
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  // --- المفتاح السحري للترجمة الشاملة (The Global Fix) ---
  // نقوم بتعديل الـ "Array Class" الأساسي في MathJax
  // هذا يضمن أن أي بيئة تعتمد على المصفوفات (Cases, Matrix, Align)
  // سترث اللغة العربية تلقائياً ولا تنساها.
  var array = TEX.Stack.Item.array;
  if (array) {
      array.Augment({
        Init: function () {
          if (arguments.callee.SUPER) arguments.callee.SUPER.apply(this, arguments);
          this.copyEnv = true; // انسخ البيئة (اللغة) من الأب
        },
        clearEnv: function () {
          ...
          // إذا كانت البيئة ممسوحة، أعد فرض اللغة العربية
