/*!
 * Arabic MathJax Extension - The Original Code + Text Fix Only
 * Keeps original matrix logic intact.
 * Adds support for \text{} un-flipping and \noar.
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* الإضافة الوحيدة في CSS: كلاس لإصلاح اتجاه النصوص */
        '.MathJax .munflip': {
          'display': 'inline-block !important',
          '-moz-transform': 'scaleX(-1)',
          '-webkit-transform': 'scaleX(-1)',
...
MathJax.Hub.Register.StartupHook('HTML-CSS Jax Ready', function () {
  MathJax.Hub.Register.StartupHook('Arabic TeX Ready', function () {
      ...
      MML[name].Augment({
        toHTML: function () {
          ...
          if (this.arabicFlipH) {
            ...
            // --- التعديل الجراحي: دعم unflip للنصوص ---
            if (this.unflip) {
    ...
    // القائمة الأصلية كما هي (بدون mtd)
    [
      'mfrac',
      'mi',
      'mn',
...
MathJax.Extension.Arabic = {
  ...
  Text: function (english, arabicText) {
    // التعديل: استخدام unflip للنصوص العربية الصريحة
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  TEX.Definitions.Add({
    macros: {
      ...
      'unflip': 'HandleUnFlipHorizontal',
      'noar': 'HandleNoArabic',
      'textar': 'HandleTextAr',
      'text': 'HandleTextAr' // استبدال text العادي
    }
  });
