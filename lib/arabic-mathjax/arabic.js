 ...
 * The MIT License
 *
 * Modified to fix Arabic Text direction inside \alwaysar
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* --- 1. بداية التعديل الذكي --- */
        /* الحالة العادية: النص يظهر بطبيعته */
        '.MathJax .munflip': {
          'display': 'inline-block !important',
          'transform': 'scaleX(1)',
          '-webkit-transform': 'scaleX(1)'
        },
        /* الحالة الخاصة: لو النص جوه عنصر معكوس (mfliph)، اعكسه تاني عشان يتعدل */
        '.MathJax .mfliph .munflip': {
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  TEX.Definitions.Add({
    macros: {
      ...
      'txt': 'HandleTxt' // --- 2. إضافة الأمر الجديد ---
    }
  });
  ...
  TEX.Parse.Augment({
    ...
    // --- دالة الأمر الجديد txt ---
    HandleTxt: function(name) {
        var textArgument = this.GetArgument(name);
        // بنحط النص في كلاس munflip
        // الـ CSS هو اللي هيقرر يعكسه ولا لأ بناء على مكانه
        var newTex = '\\class{munflip}{\\text{' + textArgument + '}}';
        this.Push(MathJax.InputJax.TeX.Parse(newTex).mml());
    },
    // -------------------------
