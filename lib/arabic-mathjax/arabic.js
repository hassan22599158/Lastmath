 ...
 * The MIT License
 *
 * Modified to fix Arabic Text readability inside \alwaysar
 * while keeping the original mirroring logic for roots and matrices.
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* --- 1. الـ CSS الذكي --- */
        /* هذا الكلاس سنضعه حول النصوص العربية */
        '.MathJax .ar-text': {
          'display': 'inline-block !important',
           /* في الوضع الطبيعي، لا تفعل شيئاً */
        },
        /* القاعدة السحرية: */
        /* إذا كان النص (ar-text) داخل عنصر مقلوب (mfliph) */
        /* قم بقلبه مرة أخرى (scaleX -1) ليصبح معدولاً للقارئ */
        '.MathJax .mfliph .ar-text': {
...
MathJax.Extension.Arabic = {
  ...
  config: MathJax.Hub.CombineConfig("Arabic", {
    ...
    identifiersMap: {
      ...
      'f': '\u0642', 'g': '\u062c\u0640', 'h': '\u0647\u0640', 'k': '\u0643', 'r': '\u0631', 't': '\u062a',
      'd': '\u062f', 'e': '\u0647\u0640', 'm': '\u0645', 'l': '\u0644',
      'sin': '\u062c\u0627', 'cos': '\u062c\u062a\u0627', 'tan': '\u0638\u0627', 'cot': '\u0638\u062a\u0627',
      'sec': '\u0642\u0627', 'csc': '\u0642\u062a\u0627', 'log': '\u0644\u0648'
    },
    numbersMap: {
      '0': '\u0660', '1': '\u0661', '2': '\u0662', '3': '\u0663', '4': '\u0664',
      '5': '\u0665', '6': '\u0666', '7': '\u0667', '8': '\u0668', '9': '\u0669'
  ...
  TeX: function (english, arabic) {
    return function (name) {
      ...
      var tex;
      if ('ar' === this.stack.env.lang) { tex = arabic; } else { tex = english; }
  ...
  Text: function (english, arabicText) {
    // نستخدم \fliph كما هي (لأننا نريد الكود الأصلي)
    // ولكن نغلف النص بـ class اسمه ar-text
    // الـ CSS سيتولى مهمة تعديل النص إذا كان مقلوباً
    return MathJax.Extension.Arabic.TeX(english, '\\fliph{\\class{ar-text}{\\text{' + arabicText + '}}}');
  },
  /* -------------------------- */
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  TEX.Parse.Augment({
    ...
    mmlToken: function (token) {
      ...
      if ('ar' === this.stack.env.lang) { this.markArabicToken(parsedToken); }
      return parsedToken;
    },
    ...
    TranslateTeX: function (name) {
      var english = this.GetArgument(name); var arabicText = this.GetArgument(name);
      var helper = Arabic.TeX(english, arabicText); return helper.call(this, name);
    },
    TranslateText: function (name) {
      var english = this.GetArgument(name); var arabicText = this.GetArgument(name);
      var helper = Arabic.Text(english, arabicText); return helper.call(this, name);
    },
    TranslateSymbols: function (name) {
      var english = this.GetArgument(name); var arabicText = this.GetArgument(name);
      var helper = Arabic.Symbols(english, arabicText); return helper.call(this, name);
    },
