 ...
 * Based on the original v1.0.0 code.
 * Patched for: Correct Text Direction & Matrix Cell Orientation.
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* كلاس التصحيح (الإضافة الوحيدة في CSS) */
        '.MathJax .munflip': {
          'display': 'inline-block !important',
          '-moz-transform': 'scaleX(-1)',
          '-webkit-transform': 'scaleX(-1)',
...
MathJax.Hub.Register.StartupHook('HTML-CSS Jax Ready', function () {
  MathJax.Hub.Register.StartupHook('Arabic TeX Ready', function () {
    ...
    [
      ...
      'mroot',
      'mtext',
      'mtd' // --- التعديل 2: إضافة mtd لتعديل أرقام المصفوفات ---
...
MathJax.Extension.Arabic = {
  ...
  Text: function (english, arabicText) {
    // --- التعديل 3: استخدام unflip بدلاً من fliph للنصوص ---
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  TEX.Definitions.Add({
    macros: {
      ...
      'unflip': 'HandleUnFlipHorizontal', // ماكرو جديد
      'noar': 'HandleNoArabic',          // ماكرو جديد
      'textar': 'HandleTextAr',          // ماكرو جديد
      'text': 'HandleTextAr',            // استبدال النص العادي
      'transx': 'TranslateTeX',
  ...
  // --- هذا هو الكود الأصلي للمصفوفات (لم ألمسه أبداً) ---
  array.Augment({
    Init: function () {
  ...
  TEX.Parse.Augment({
    ...
    // --- الدوال الإضافية (Macros Handlers) ---
