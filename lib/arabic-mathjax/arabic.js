 ...
 *
 * Modified slightly to fix text mirroring while keeping original library logic intact.
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* --- 1. كلاس جديد: وظيفته الوحيدة تعديل النص المشقلب --- */
        '.MathJax .unflip-text': {
          'display': 'inline-block !important',
          'transform': 'scaleX(-1)',       /* اقلب القلبة، فيرجع معدول */
          '-webkit-transform': 'scaleX(-1)',
          '-moz-transform': 'scaleX(-1)'
        },
        /* ---------------------------------------------------- */
...
MathJax.Extension.Arabic = {
  ...
  /* --- 2. التعديل الوحيد في الدوال: دالة النص --- */
  Text: function (english, arabicText) {
    // الكود الأصلي كان: return MathJax.Extension.Arabic.TeX(english, '\\fliph{\\text{' + arabicText + '}}');
    // إحنا سبنا \fliph زي ما هي (عشان تحافظ على مكانها في المعادلة)
    // بس حطينا النص جوه unflip-text عشان الحروف تظهر معدولة
    return MathJax.Extension.Arabic.TeX(english, '\\fliph{\\class{unflip-text}{\\text{' + arabicText + '}}}');
  },
  /* ---------------------------------------------- */
