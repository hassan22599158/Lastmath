 ...
 *
 * Modified to add \txt command for readable Arabic text.
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* --- 1. كلاس جديد خاص بالأمر txt --- */
        /* الوضع الافتراضي: لا تفعل شيئاً (يظهر النص كما هو) */
        '.MathJax .ar-txt-fix': {
          'display': 'inline-block !important'
        },
        /* الحالة الخاصة: إذا كان النص داخل حاوية مقلوبة (mfliph) */
        /* قم بعكس النص مرة أخرى (-1) ليصبح مقروءاً */
        '.MathJax .mfliph .ar-txt-fix': {
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  TEX.Definitions.Add({
    macros: {
      ...
      'txt': 'HandleTxt'  // --- 2. تسجيل الأمر الجديد txt ---
    }
  });
  ...
  TEX.Parse.Augment({
    ...
    // --- 3. برمجة دالة الأمر txt ---
    HandleTxt: function(name) {
        // نقرأ النص المكتوب داخل القوسين
        var arg = this.GetArgument(name);
        
        // نقوم بعمل كود TeX جديد يضع النص داخل الكلاس الجديد
        // \class{ar-txt-fix}{\text{...}}
        // لاحظ أننا لم نستخدم \fliph هنا، تركنا التحكم للـ CSS فقط
        var tex = '\\class{ar-txt-fix}{\\text{' + arg + '}}';
        
        // نقوم بتحويل هذا الكود إلى MathML
        this.Push(MathJax.InputJax.TeX.Parse(tex).mml());
    },
    // ------------------------------------
