/*!
 * Arabic MathJax Extension - Final Version (v3.0.0)
 * Features: Correct RTL Layout, Smart Text Un-flipping, Crash Protection.
 */

// 1. حقن أنماط CSS (القلب والتصحيح)
MathJax.Hub.Register.StartupHook('HTML-CSS Jax Require', function () {
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        // كلاس القلب الأساسي (للمعادلات والرموز)
        '.MathJax .mfliph': {
        ...
        // كلاس "إلغاء القلب" (للنصوص والاستثناءات)
        // يقوم بقلب العنصر مرة أخرى ليعود لشكله الطبيعي مع الحفاظ على مكانه
        '.MathJax .munflip': {
          'display': 'inline-block !important',
          '-moz-transform': 'scaleX(-1)',
          '-webkit-transform': 'scaleX(-1)',
...
// 2. تعديل عناصر MathJax لتقبل القلب
MathJax.Hub.Register.StartupHook('HTML-CSS Jax Ready', function () {
  MathJax.Hub.Register.StartupHook('Arabic TeX Ready', function () {
      ...
      MML[name].Augment({
        toHTML: function () {
          ...
          if (this.arabicFlipH) {
            ...
            // المنطق الذكي: إذا كان العنصر يحمل علامة unflip نستخدم كلاس التصحيح
            if (this.unflip) {
    ...
    // تطبيق التعديل على جميع العناصر الرياضية
    [
...
MathJax.Extension.Arabic = {
  ...
  Text: function (english, arabicText) {
     return MathJax.Extension.Arabic.TeX(english, '\\textar{' + arabicText + '}');
  }
};

MathJax.Hub.Startup.signal.Post('Arabic TeX Startup');

// 4. منطق المعالجة (Core Parsing Logic)
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  // --- تعريف الماكروس (هام جداً لمنع الانهيار) ---
  TEX.Definitions.Add({
    macros: {
      'ar': 'HandleArabic',
      ...
      'textar': 'HandleTextAr'     // ماكرو النص الذكي
    }
  });
  ...
  TEX.Parse.Augment({
    ...
    HandleNoArabic: function (name) {
        ...
        // إذا كنا في وضع عربي، نفعّل unflip للحفاظ على الاتجاه
        if ('ar' === this.stack.env.lang) {
             arg.unflip = true;
    ...
    HandleTextAr: function(name) {
        ...
        // إذا كنا في وضع عربي، نفعّل unflip للنص
        if ('ar' === this.stack.env.lang) {
             mtext.unflip = true;
