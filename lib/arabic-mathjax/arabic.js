 ...
 *
 * Modified to include \txt command for correct Arabic text rendering.
 */
  ...
  MathJax.Hub.Config({
    'HTML-CSS': {
      styles: {
        ...
        /* --- 1. كلاس جديد لعكس النص مرة أخرى ليصبح مقروءاً --- */
        '.MathJax .munflip': {
          'display': 'inline-block !important',
          '-moz-transform': 'scaleX(-1)',
          '-webkit-transform': 'scaleX(-1)',
...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function () {
  ...
  TEX.Definitions.Add({
    macros: {
      ...
      'txt': 'HandleTxt' // --- 2. تسجيل الأمر الجديد ---
    }
  });
  ...
  TEX.Parse.Augment({
    ...
    // --- 3. دالة معالجة الأمر txt ---
    HandleTxt: function(name) {
        var textArgument = this.GetArgument(name);
        // نقوم بإنشاء TeX يغلف النص بـ class munflip ثم يقوم بعكسه بـ fliph
        // fliph يضعه في اليمين، و munflip يصحح قراءة النص
        var newTex = '\\fliph{\\class{munflip}{\\text{' + textArgument + '}}}';
